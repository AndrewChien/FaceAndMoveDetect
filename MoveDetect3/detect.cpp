...
void  update_mhi(IplImage* img, IplImage* dst, int diff_threshold,cv::Mat frame, time_t timep)
{
	...
	for (i = 0; i < seq->total; i++)
	{
		...
		cvCircle(dst, center, cvRound(magnitude*1.2), color, 3, CV_AA, 0);//画圆圈住动态点
		cvLine(dst, center, cvPoint(cvRound(center.x + magnitude*cos(angle*CV_PI / 180)), cvRound(center.y - magnitude*sin(angle*CV_PI / 180))), color, 3, CV_AA, 0);//画线标明方向

		//以下抓拍动态
		//double t = difftime(timep, lasttime);
		//if (t > 1)//每1.5秒只能触发1次
		//{
			lasttime = timep;
			strftime(tmp, sizeof(tmp), "%Y%m%d%H%M%S.jpg", localtime(&timep));//获取当前时间
			imwrite(tmp, frame);
		//}
	}
}
...
{
	cv::VideoCapture vcapture(0);//用于实时视频
	IplImage* motion = 0;
	CvCapture* capture = 0;
	time_t timep;
	time(&timep);//获取当前时间反写
	...
	if (capture)
	{
		...
		for (;;)
		{
			...
			update_mhi(image, motion, 60, frame, timep);
